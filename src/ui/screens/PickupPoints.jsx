...
export function PickupPoints() {
  const { toast, showDialog, t, tf, te } = useUi();
  ...
  async function onRemove(p) {
    ...
    if (count > 0) {
      showDialog({
        title: t('pickupCannotDelete'),
        body: tf('pickupCannotDeleteBody', { name: p.name, count }),
        confirmLabel: t('markInactive'),
        onConfirm: async () => {
          await removePickupPoint(p.id);
          toast(tf('pointNowInactive', { name: p.name }));
        }
      });
      return;
    }
    showDialog({
      title: tf('removePointTitle', { name: p.name }),
      body: t('removePointBody'),
      confirmLabel: t('yesRemove'),
      danger: true,
      onConfirm: async () => {
        await removePickupPoint(p.id);
        toast(tf('pointRemoved', { name: p.name }));
      }
    });
  }
  ...
  return (
    <div style="min-height:100vh;display:flex;flex-direction:column">
      ...
      <div class="main-scroll scr">
        ...
        {pickups.map((p) => (
          <div key={p.id} style={`padding:12px 14px;border-bottom:1px solid var(--color-neutral-300);display:flex;gap:11px;align-items:flex-start;${p.active ? '' : 'opacity:.5'}`}>
            <div style="flex:1;min-width:0">
              ...
              <div style="font-size:11.5px;color:var(--color-neutral-700);margin-top:2px">{p.area || t('noLandmark')} · {p.distance_km || 0} {t('kmUnit')}</div>
        ...
        {pickups.length === 0 && <div class="empty-state">{t('noPickupsYet')}</div>}
          ...
          <div class="dialog-sheet">
            ...
            <div style="padding:14px;display:flex;flex-direction:column;gap:13px">
              <div class="field">
                ...
                {errors.name && <div class="field-error">{te(errors.name)}</div>}
              </div>
              <div class="field">
                <label>{t('areaLandmark')}</label>
                <input class="input" value={sheet.area} onInput={(e) => setSheet({ ...sheet, area: e.currentTarget.value })} />
              </div>
              <div class="field">
                <label>{t('monthlyFareLabel')}</label>
                ...
                {errors.monthly_fare && <div class="field-error">{te(errors.monthly_fare)}</div>}
              </div>
              <div class="field">
              ...
              >
                <span style="flex:1;font-size:13px;font-weight:700">{sheet.active ? t('pointActiveLabel') : t('pointInactiveLabel')}</span>
